const FetchCashierDailyReceipts = (res, cashier_id, dbConn) => {
  ...
  const query =
    "SELECT COUNT(receipt_id) AS receipts FROM receipt WHERE cashier_id=? AND YEAR(date_added)=? AND MONTH(date_added)=? AND DAY(date_added)=?";

  dbConn.query(
    query,
    [cashier_id, date.getFullYear(), date.getMonth() + 1, date.getDate()],
    (error, results) => {
      if (error) {
        console.log(error);
