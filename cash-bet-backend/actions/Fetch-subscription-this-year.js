const FetchSubscriptionThisYear = (res, dbConn, user_role, user_id) => {
  ...
  if (user_role === "partner") {
    query =
      "SELECT SUM(credit.credit_amount) AS total_credit FROM credit RIGHT JOIN user ON user.user_id=credit.given_to WHERE credit.credit_type=? AND user.user_role=? AND YEAR(credit.transaction_date)=? AND given_to=?";
  } else {
    query =
  ...
  dbConn.query(
    query,
    ["plus", "partner", new Date().getFullYear(), user_id],
    (error, results) => {
      if (error) {
